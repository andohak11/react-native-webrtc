...
- (void)startRunning
{
    ...
    NSLog(@"VIDEORECORDER: startRunning...");
...
- (void)stopRunning
{
    ...
    NSLog(@"VIDEORECORDER: stopRunning...");
...
- (AVCaptureSession *)setupCaptureSession
{
    ...
    //if(![self addDefaultCameraInputToCaptureSession:captureSession]){
        NSLog(@"VIDEORECORDER: failed to add camera input to capture session");
    //}
    if(![self addDefaultMicInputToCaptureSession:captureSession]){
        NSLog(@"VIDEORECORDER: failed to add mic input to capture session");
    }
...
- (BOOL)addDefaultCameraInputToCaptureSession:(AVCaptureSession *)captureSession
{
    ...
    if(error){
        NSLog(@"VIDEORECORDER: error configuring camera input: %@", [error localizedDescription]);
        return NO;
    } else {
...
- (BOOL)addCameraAtPosition:(AVCaptureDevicePosition)position toCaptureSession:(AVCaptureSession *)captureSession
{
    ...
    if(!cameraDeviceInput){
        NSLog(@"VIDEORECORDER: No capture device found for requested position");
        return NO;
    }
...
- (BOOL)addDefaultMicInputToCaptureSession:(AVCaptureSession *)captureSession
{
    ...
    if(error){
        NSLog(@"VIDEORECORDER: error configuring mic input: %@", [error localizedDescription]);
        return NO;
    } else {
...
- (BOOL)addInput:(AVCaptureDeviceInput *)input toCaptureSession:(AVCaptureSession *)captureSession
{
    if([captureSession canAddInput:input]){
        ...
        NSLog(@"VIDEORECORDER: can't add input: %@", [input description]);
    }
    return NO;
}
...
- (BOOL)addOutput:(AVCaptureOutput *)output toCaptureSession:(AVCaptureSession *)captureSession
{
    if([captureSession canAddOutput:output]){
        ...
        NSLog(@"VIDEORECORDER: can't add output: %@", [output description]);
    }
    return NO;
}
